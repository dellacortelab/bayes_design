...
def beam_decode(prob_model, struct, seq, decode_order, fixed_position_mask, from_scratch, n_beams):
    ...
    for j, decode_idx in enumerate(decode_order):
        ...
        for i in range(0, len(seqs), n_concurrent_seqs):
            probs = prob_model(seq=seqs[i:i + n_concurrent_seqs], struct=struct, decode_order=decode_order, token_to_decode=torch.tensor([decode_idx]).expand(len(seqs[i:i + n_concurrent_seqs])), mask_type=mask_type)
